...
function get_sub_nav() {
	...
	$menu .= '<ul>
		<li '. (!isset($_GET['area']) ? 'class="active"' : '') .'><a href="'. SITE_URL . APP_PATH .'/index.php'. (!empty($p) ? $p : '') .'">Dashboard</a></li>';
	
	// Loops through each nav item
	foreach ($page['areas'] as $area => $nav_item) {
	
		$menu .= '<li '. (isset($_GET['area']) && $_GET['area'] == $area ? 'class="active"' : '') .'><a href="'. SITE_URL . APP_PATH .'/index.php'. (!empty($p) ? $p .'&' : '?') .'area='. $area .'">'. $nav_item[0] .'</a></li>';
